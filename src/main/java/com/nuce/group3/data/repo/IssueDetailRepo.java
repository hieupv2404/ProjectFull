...
public interface IssueDetailRepo extends JpaRepository<IssueDetail, Integer> {
    ...
    List<IssueDetail> findIssueDetailByFilter(@Param(value = "priceTotalFrom") BigDecimal priceTotalFrom, @Param(value = "priceTotalTo") BigDecimal priceTotalTo, @Param(value = "imei") String imei, @Param(value = "issueCode") String issueCode, @Param(value = "productInfo") String productInfo, Pageable pageable);
    ...
    @Query(value = "select i.*" +
            ...
            " and (:issueCode is null or i.issue_id in (select s.id from issue s where s.active_flag=1 and s.code like %:issueCode%))" +
            " and (:productInfo is null or i.product_id in (select p.id from product_info p where p.active_flag =1 and p.name like %:productInfo%)) ", nativeQuery = true)
    List<IssueDetail> findIssueDetailForExport(@Param(value = "priceTotalFrom") BigDecimal priceTotalFrom, @Param(value = "priceTotalTo") BigDecimal priceTotalTo, @Param(value = "imei") String imei, @Param(value = "issueCode") String issueCode, @Param(value = "productInfo") String productInfo);
