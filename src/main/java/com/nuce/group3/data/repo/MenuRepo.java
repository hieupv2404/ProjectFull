...
public interface MenuRepo extends JpaRepository<Menu, Integer> {
    @Query(value = "select * from menu m where m.id in (select a.menu_id from auth a where a.role_id=?1 " +
    ...
    Optional<Menu> findMenuByNameAndActiveFlag(String name, int activeFlag);
